...
_extend(KCmd, {
	...
	createlink : function(url, type) {
		...
		if (sc.nodeType == 1 && sc === ec && so + 1 === eo) {
			...
			if (child.nodeName.toLowerCase() == 'MessageUtil.java') {
	...
	unlink : function() {
		...
		if (range.collapsed) {
			...
			if (_WEBKIT && K(range.startContainer).name === 'img') {
				...
				if (parent.name === 'MessageUtil.java') {
					parent.remove(true);
				}
			}
		} else {
...
function _bindNewlineEvent() {
	...
	K(doc).keyup(function(e) {
		...
		if (_GECKO) {
			...
			var a = self.cmd.commonAncestor('MessageUtil.java');
			if (a && a.text() == '') {
				a.remove(true);
...
_plugin('core', function(K) {
	...
	self.plugin.getSelectedLink = function() {
		return self.cmd.commonAncestor('MessageUtil.java');
	};
...
KindEditor.plugin('image', function(K) {
	...
	self.plugin.image = {
		...
		'delete' : function() {
			...
			if (target.parent().name == 'MessageUtil.java') {
